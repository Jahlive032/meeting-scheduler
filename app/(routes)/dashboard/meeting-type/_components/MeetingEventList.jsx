  ...
  return (
    <div className='mt-10 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-7'>
        {eventList.length > 0 ? eventList?.map((event, index) =>(
            <div className='border shadow-md border-t-8 rounded-lg p-5 flex flex-col gap-3' style={{borderTopColor:event?.themeColor}}>
                <div className='flex justify-end'>                   
                    <DropdownMenu>
                        ...
                        <DropdownMenuContent>
                            <DropdownMenuItem className="flex gap-2"><Pen/> Editer</DropdownMenuItem>
                            <DropdownMenuItem className="flex gap-2" onClick={() => onDeleteMeetingEvent(event)}><Trash/> Supprimer</DropdownMenuItem>
                        </DropdownMenuContent>
                    </DropdownMenu>
                </div>
                <h2 className='font-medium text-xl'>{event?.eventName}</h2>
                <div className='flex justify-between'>
                    <h2 className='flex gap-2 text-gray-500'><Clock/>{event.duration} Minutes</h2>
                    <h2 className='flex gap-2 text-gray-500'><MapPin/>{event.locationType}</h2>
                </div>
                <hr />
                <div className='flex justify-between'>
                    <h2 className='flex gap-2 text-sm items-center text-primary cursor-pointer' onClick={() => {
                        onCopyClickHandler(event);
                    }}><Copy className='h-4 w-4'/>Copier le lien ici</h2>
                    <Button variant="outline" className="border-primary rounded-full text-primary">Partager</Button>
                </div>
            </div>
        )) : <h2>Chargement...</h2>}
    </div>
